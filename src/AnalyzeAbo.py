...
def findPolymorphismsInReads(referenceFileName, readsFileName, outputDirectoryName):
    ...
    analyzeChosenPolymorphicPositions(referenceFileName, outputDirectoryName, sequenceStats)
...
def alignSequencesAgainstReference(referenceLocation, alleleFileLocation, outputDirectory):
    ...
    try:
        ...
        newReferenceLocation = join(alignmentSubdir,'AlignmentReference.fasta')
...
def printBasePolymorphisms(alignmentSummaryFile, referencePosition0Based, currentSequenceStats):
    ...
    alignmentSummaryFile.write('Aligned Read Count:' + str(currentSequenceStats.alignedReadCount) + '\n')
    
    # Header, blood group is in the columns.
    alignmentSummaryFile.write('Mat\tMis\tIns\tDel\tA\tG\tC\tT\n')
    alignmentSummaryFile.write(str(int(currentSequenceStats.getMatchPercent())) + '\t')
    alignmentSummaryFile.write(str(int(currentSequenceStats.getMismatchPercent())) + '\t')
    alignmentSummaryFile.write(str(int(currentSequenceStats.getInsertionPercent())) + '\t')
    alignmentSummaryFile.write(str(int(currentSequenceStats.getDeletionPercent())) + '\t')
    # loop Nucleotides
    for nucleotide in 'A', 'G', 'C', 'T':
        nucleotidePercentage = currentSequenceStats.getNucleotidePercentage(nucleotide)
...
def findReadPolymorphisms(outputDirectory, sequenceStats):
    ...
    alignmentSummaryFile = createOutputFile(join(outputDirectory,'ABOReadPolymorphisms.txt')) 
    ...
    for referencePosition0Based, currentSequenceStats in enumerate(sequenceStats):
        ...
        if (matchPercent > ReadCutoffAgreementPercent):
            ...
            printBasePolymorphisms(alignmentSummaryFile, referencePosition0Based, currentSequenceStats)
            ...
            try:
                ...
                for i in range(0,baseCount):
                    leftSequence += sequenceStats[referencePosition0Based - baseCount + i].referenceBase
                    rightSequence += sequenceStats[referencePosition0Based + i].referenceBase
...
def analyzeChosenPolymorphicPositions(referenceFileName, outputDirectory, sequenceStats):
    ...
    phenotypeOutputFile = createOutputFile(join(outputDirectory,'ABOPhenotype.txt')) 
    ...
    if(len(alignmentRef)==135):
        ...
        printBasePolymorphisms(phenotypeOutputFile, 21, sequenceStats[21]) # pass the 0-based
    ...
    # Exon 7
        ...
        phenotypeOutputFile.write('This exon has length:' + str(len(alignmentRef)))
...
def writeAlleleAlignmentSpreadsheet(outputDirectory, myBloodGroupStats):
    ...
    alignmentSummaryFile = createOutputFile(join(outputDirectory,'AlignmentSpreadsheet.csv'))
    # headers: 
    alignmentSummaryFile.write('Ref_Position_1based,Ref_Base'
...
def findInterestingAllelePolymorphisms(outputDirectory, myBloodGroupStats):
    ...
    alignmentSummaryFile = createOutputFile(join(outputDirectory,'Interesting_Polymorphisms.txt')) 
   
   
    for referencePosition0Based, referenceBase in enumerate(myBloodGroupStats.referenceSequence):

        aMatchPercent = myBloodGroupStats.getMatchPercent('A',referencePosition0Based)  
        bMatchPercent = myBloodGroupStats.getMatchPercent('B',referencePosition0Based)  
        oMatchPercent = myBloodGroupStats.getMatchPercent('O',referencePosition0Based) 
        ...
        if(aMatchPercent > AlleleCutoffAgreementPercent 
            ...
            for bloodGroup in ('A','B','O'):
                ...
                alignmentSummaryFile.write(str(int(myBloodGroupStats.getMatchPercent(bloodGroup, referencePosition0Based))) + '\t')
                alignmentSummaryFile.write(str(int(myBloodGroupStats.getMismatchPercent(bloodGroup, referencePosition0Based))) + '\t')
                alignmentSummaryFile.write(str(int(myBloodGroupStats.getInsertionPercent(bloodGroup, referencePosition0Based))) + '\t')
                alignmentSummaryFile.write(str(int(myBloodGroupStats.getDeletionPercent(bloodGroup, referencePosition0Based))) + '\t')
    
                # loop Nucleotides
                for nucleotide in ('A','G','C','T'):
                    
                    nucleotidePercentage = myBloodGroupStats.getNucleotidePercentage(bloodGroup, nucleotide, referencePosition0Based)
                    percentageInt = int(nucleotidePercentage)
